...
from ithacaweb.base.blocks import BaseStreamBlock
...
class ProjectElement(models.Model):
    ...
    available to Wagtail called the ParentalManyToManyField on the ProjectPage
    ...
    class Meta:
        verbose_name_plural = 'Project elements'


@register_snippet
class ProjectType(models.Model):
    """
    A Django model to define the project type
    It uses the `@register_snippet` decorator to allow it to be accessible
    via the Snippets UI. In the ProjectPage model you'll see we use a ForeignKey
    to create the relationship between ProjectType and ProjectPage. This allows a
    single relationship (e.g only one ProjectType can be added) that is one-way
    (e.g. ProjectType will have no way to access related ProjectPage objects)
    """
...
class ProjectPage(Page):
    ...
    # and they both have a FK to project_type, they'll both try to create a
    ...
    elements = ParentalManyToManyField('ProjectElement', blank=True)
